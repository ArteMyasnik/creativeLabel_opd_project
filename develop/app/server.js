...
app.get('/stats_test_visual_signal/:login', async (req, res) => {
    ...
    try {
        ...
        const testsData = result.rows.map((test, index) => {
            ...
            return {
                ...
                avgReaction: reactions.length > 0
                    ? (reactions.reduce((sum, a) => sum + a, 0) / reactions.length).toFixed(1)
                    : 0,
        ...
        const avgReaction = totalAttempts > 0
            ? (allReactions.reduce((sum, a) => sum + a, 0) / totalAttempts).toFixed(1)
            : 0;
...
app.get('/stats_test_sound_signal/:login', async (req, res) => {
    ...
    try {
        ...
        const allReactions = testsData.flatMap(test => test.reactions);
        const totalAttempts = allReactions.length;
        const totalMissed = testsData.reduce((sum, test) => sum + test.missedSignals, 0);
        const totalSignals = totalAttempts + totalMissed;
        const avgReaction = totalAttempts > 0
            ? (allReactions.reduce((sum, a) => sum + a, 0) / totalAttempts).toFixed(1)
            : 0;
        const totalAccuracy = totalSignals > 0
            ? ((totalAttempts / totalSignals) * 100).toFixed(1)
            : 0;
        ...
        res.render('tests/stats/stats_test_sound_signal', {
            ...
            stats: {
                ...
                bestReaction: totalAttempts > 0 ? Math.min(...allReactions) : 0,
                worstReaction: totalAttempts > 0 ? Math.max(...allReactions) : 0,
                avgTestDuration: (testsData.reduce((sum, test) => sum + test.testDuration, 0) / testsData.length)
            },
            testsData, // передаем данные по каждому тесту отдельно
            chartData: {
                testNumbers: testsData.map(test => test.testNumber),
...
app.get('/stats_test_color_signal/:login', async (req, res) => {
    ...
    try {
        ...
        const testsData = result.rows.map((test, index) => {
            const reactions = test.reaction_times || [];
            const totalClicks = test.correct_answers + test.missed_clicks + test.wrong_clicks;
        ...
        const allReactions = testsData.flatMap(test => test.reactions);
        const totalAttempts = allReactions.length;
        const totalCorrect = testsData.reduce((sum, test) => sum + test.correctAnswers, 0);
        const totalWrong = testsData.reduce((sum, test) => sum + test.wrongClicks, 0);
        const totalMissed = testsData.reduce((sum, test) => sum + test.missedClicks, 0);
        const totalClicks = totalCorrect + totalWrong + totalMissed;
        const totalAccuracy = totalClicks > 0 ? Math.round((totalCorrect / totalClicks) * 100) : 0;
        const avgReaction = totalAttempts > 0
            ? (allReactions.reduce((sum, a) => sum + a, 0) / totalAttempts).toFixed(1)
            : 0;
        ...
        res.render('tests/stats/stats_test_color_signal', {
            ...
            stats: {
                ...
                totalMissed,
                totalAccuracy,
                avgTestDuration: (testsData.reduce((sum, test) => sum + test.testDuration, 0) / testsData.length).toFixed(1)
            },
            testsData,
            chartData: {
                testNumbers: testsData.map(test => test.testNumber),
...
app.get('/stats_test_digital_signal/:login', async (req, res) => {
    ...
    try {
        ...
        // 2. Получаем все попытки пользователя для этого теста
        const result = await pool.query(
            `SELECT id, reaction_times, correct_answers, wrong_answers, 
                    missed_attempts, test_duration, created_at
        ...
        const testsData = result.rows.map((test, index) => {
            const reactions = test.reaction_times || [];
            const totalQuestions = test.correct_answers + test.wrong_answers + test.missed_attempts;
        ...
        const allReactions = testsData.flatMap(test => test.reactions);
        const totalAttempts = allReactions.length;
        const totalQuestions = testsData.reduce((sum, test) => sum + test.totalQuestions, 0);
        const totalCorrect = testsData.reduce((sum, test) => sum + test.correctAnswers, 0);
        ...
        res.render('tests/stats/stats_test_digital_signal', {
            ...
            stats: {
                ...
                totalMissed: testsData.reduce((sum, test) => sum + test.missedAttempts, 0),
...
app.get('/stats_test_audio_sum/:login', async (req, res) => {
    ...
    try {
        ...
        const totalAttempts = allReactions.length;
        const totalCorrect = testsData.reduce((sum, test) => sum + test.correctAnswers, 0);
        const totalWrong = testsData.reduce((sum, test) => sum + test.wrongAnswers, 0);
    ...
    try {
        ...
        const testsData = result.rows.map((test, index) => {
            ...
            return {
                ...
                testDuration: test.test_duration,
                createdAt: test.created_at,
                avgSignedError: signedErrors.length > 0
                    ? (signedErrors.reduce((sum, a) => sum + a, 0) / signedErrors.length).toFixed(2)
                    : 0,
                avgAbsoluteError: absoluteErrors.length > 0
                    ? (absoluteErrors.reduce((sum, a) => sum + a, 0) / absoluteErrors.length).toFixed(2)
                    : 0,
                bestResult: absoluteErrors.length > 0 ? Math.min(...absoluteErrors).toFixed(2) : 0,
                worstResult: absoluteErrors.length > 0 ? Math.max(...absoluteErrors).toFixed(2) : 0,
                totalAttempts: attempts.length,
                prematureCount: signedErrors.filter(e => e > 0).length,
                delayedCount: signedErrors.filter(e => e < 0).length,
                stdDeviation: signedErrors.length > 0
                    ? Math.sqrt(signedErrors.reduce((sum, a) => sum + Math.pow(a - (signedErrors.reduce((sum, a) => sum + a, 0) / signedErrors.length), 2), 0) / signedErrors.length).toFixed(2)
                    : 0
            };
        });
        ...
        res.render('tests/stats/stats_test_simple_rdo', {
            ...
            stats: {
                totalTests: testsData.length,
                totalAttempts,
                avgSignedError: totalAttempts > 0
                    ? (allSignedErrors.reduce((sum, a) => sum + a, 0) / totalAttempts).toFixed(2)
                    : 0,
                avgAbsoluteError: totalAttempts > 0
                    ? (allAbsoluteErrors.reduce((sum, a) => sum + a, 0) / totalAttempts).toFixed(2)
                    : 0,
                bestResult: totalAttempts > 0 ? Math.min(...allAbsoluteErrors).toFixed(2) : 0,
                worstResult: totalAttempts > 0 ? Math.max(...allAbsoluteErrors).toFixed(2) : 0,
                prematureCount: allSignedErrors.filter(e => e > 0).length,
                delayedCount: allSignedErrors.filter(e => e < 0).length,
                stdDeviation: totalAttempts > 0
                    ? Math.sqrt(allSignedErrors.reduce((sum, a) => sum + Math.pow(a - (allSignedErrors.reduce((sum, a) => sum + a, 0) / totalAttempts), 2), 0) / totalAttempts).toFixed(2)
                    : 0
            },
            testsData,
            chartData: {
    ...
    try {
        ...
        const testsData = result.rows.map((test, index) => {
            ...
            const avgReaction = allReactions.length > 0
                ? (allReactions.reduce((sum, a) => sum + Math.abs(a), 0) / allReactions.length).toFixed(1)
                : 0;

            const bestReaction = allReactions.length > 0
                ? Math.min(...allReactions.map(Math.abs))
                : 0;

            const worstReaction = allReactions.length > 0
                ? Math.max(...allReactions.map(Math.abs))
                : 0;
        ...
        const totalAttempts = allReactions.length;
        const avgReaction = totalAttempts > 0
            ? (allReactions.reduce((sum, a) => sum + Math.abs(a), 0) / totalAttempts).toFixed(1)
            : 0;
        ...
        res.render('tests/stats/stats_test_complex_rdo', {
            ...
            stats: {
                ...
                totalAttempts,
                avgReaction,
                bestReaction: totalAttempts > 0 ? Math.min(...allReactions.map(Math.abs)) : 0,
                worstReaction: totalAttempts > 0 ? Math.max(...allReactions.map(Math.abs)) : 0
            },
            testsData,
            chartData: {
                testNumbers: testsData.map(test => test.testNumber),
...
function calculateCircleStats(responses, speed) {
    ...
    const avgError = responses.reduce((sum, val) => sum + val, 0) / responses.length;
    const avgAbsError = responses.reduce((sum, val) => sum + Math.abs(val), 0) / responses.length;

    const variance = responses.reduce((sum, val) => sum + Math.pow(val - avgError, 2), 0) / responses.length;
